...
describe('Atlas public URL and task-order contract', () => {
  it('keeps the documented query state behind strict public enums and IDs', async () => {
    ...
    expect(source).toMatch(/createAtlasRouteStateCodec/)
    expect(source).toMatch(/atlasRouteState\.hydrate\(routeQuery\)/)
    expect(source).toMatch(/atlasRouteState\.serialize/)
    expect(source).toMatch(/atlasRouteState\.matches\(canonicalQuery, routeQuery\)/)
