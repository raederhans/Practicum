...
## Exact candidate, publication, and cleanup evidence

- Five normal Lore commits were created from base `ca8292040a402eae1d2e461708a4cc912867efcb`: UI `e45c5ab`, P2/P3 plus Actions `215bafe`, P1 acquisition/receipts `75663d2`, platform boundary `0cfbc9c`, and umbrella integration records `bd91194`.
- The fixed product candidate is `bd91194f85c6cc8ce1fc3d6ced80dd66d4bf6511`. After it was fixed, no product code changed. Fresh exact-SHA gates passed: Python `130 passed, 7 subtests`; reviewed-modeling ready; full-upstream blocked by the same seven explicit gaps; public 167/167, audit 0, build/verifier, and two identical local Pages-base manifests; Dashboard 24/24, audit 0, bundle analysis, and build.
- Exact-SHA local browser admission passed ten public route-width checks, H1 route focus, forced-colors, local-only origins, and zero console warnings/errors. Dashboard kept the preview absent before viewport entry and loaded it after entry; keyboard Enter reached a real canvas with `data-map-ready=true` and no data error. A fast chained-navigation focus false negative reproduced only while the previous 180 ms transition was still active; stable user-sequence waits passed three of three without a product change.
- A normal push moved `origin/main` to the exact product candidate with divergence `0/0`. GitHub Actions run `31352969379` used that exact `headSha`; its build and Pages deploy jobs succeeded.
- Live Pages at `https://raederhans.github.io/Practicum/` serves a schema-v2 manifest with SHA-256 `288f18bed58d9c5aa9a5a4a5b53bbec2b2885a617d56ae40f7e0f52c101293a6`; all 11 declared live files matched their live byte counts and SHA-256 values. Live browser admission repeated the ten route-width, focus, forced-colors, same-origin, and zero-console checks successfully.
- The local Windows Pages manifest SHA-256 is `22d5da7cd40635e34e081fbf3e5823bbba7077c23e3bb36fdf4d647c2aee0ff2`; Linux CI differs only for the two copied text artifacts `index.html` and `observatory-mark.svg` because checkout/build whitespace differs. The live manifest is internally exact, but cross-platform bit-for-bit build identity remains an explicit non-blocking reproducibility gap.
- The four verified P1 assets and their receipts were copied to the main ignored owner cache `cache/p1-authorized-source-acquisition-20260810/downloads/`. All four destination files were rehashed against their receipts: 1,157,714,975 asset bytes, 4/4 verified. Earth Engine venv/logs and the failed OSM partial were not copied.
- Git worktree registrations `fa7d`, `0313`, `aefe`, and `1335` were removed after recovery was established through main. Windows/Codex kept four empty directory shells, but each contains zero files, zero subdirectories, no `.git`, and is no longer registered. The unrelated `591a` worktree remains registered and untouched.
- Main-owned browser sessions were closed; exact owned Vite listener PIDs were stopped after command/port verification; ports 43261/43262 are free. Generated package/browser/temp artifacts are removed in final cleanup. The three personal-research files remain untracked, unmodified, and unstaged.
